...
parseFloat(String(O).replace(k,"")):0}:y=="date"||y=="datetime"?function(O){return a.jgrid.parseDate(B,O).getTime()}:a.isFunction(y)?y:function(O){O||(O="");return a.trim(String(O).toUpperCase())};a.each(m,function(O,Q){A=a.jgrid.getAccessor(Q,p);if(A===undefined)A="";A=G(A);I.push({vSort:A,index:O})});I.sort(function(O,Q){O=O.vSort;Q=Q.vSort;return f._compare(O,Q,w)});y=0;for(var M=m.length;y<M;){v=I[y].index;C.push(m[v]);y++}return C};this._getGroup=function(m,p,v,y,B){var C=[],I=null,w=null,A;
a.each(f._getOrder(m,p,v,y,B),function(G,M){A=a.jgrid.getAccessor(M,p);if(A===undefined)A="";if(!f._equals(w,A)){w=A;I!=null&&C.push(I);I=f._group(p,A)}I.items.push(M)});I!=null&&C.push(I);return C};this.ignoreCase=function(){h=false;return f};this.useCase=function(){h=true;return f};this.trim=function(){j=true;return f};this.noTrim=function(){j=false;return f};this.combine=function(m){var p=a.from(g);h||p.ignoreCase();j&&p.trim();result=m(p).showQuery();f._append(result);return f};this.execute=function(){var m=
c,p=[];if(m===null)return f;a.each(g,function(){eval(m)&&p.push(this)});g=p;return f};this.data=function(){return g};this.select=function(m){f._performSort();if(!f._hasData())return[];f.execute();if(a.isFunction(m)){var p=[];a.each(g,function(v,y){p.push(m(y))});return p}return g};this.hasMatch=function(){if(!f._hasData())return false;f.execute();return g.length>0};this.showQuery=function(m){var p=c;if(p===null)p="no query found";if(a.isFunction(m)){m(p);return f}return p};this.andNot=function(m,
p,v){n=!n;return f.and(m,p,v)};this.orNot=function(m,p,v){n=!n;return f.or(m,p,v)};this.not=function(m,p,v){return f.andNot(m,p,v)};this.and=function(m,p,v){r=" && ";if(m===undefined)return f;return f._repeatCommand(m,p,v)};this.or=function(m,p,v){r=" || ";if(m===undefined)return f;return f._repeatCommand(m,p,v)};this.isNot=function(m){n=!n;return f.is(m)};this.is=function(m){f._append("this."+m);f._resetNegate();return f};this._compareValues=function(m,p,v,y,B){var C;C=t?"this."+p:"this";if(v===
undefined)v=null;v=v===null?p:v;switch(B.stype===undefined?"text":B.stype){case "int":case "integer":v=isNaN(Number(v))?"0":v;C="parseInt("+C+",10)";v="parseInt("+v+",10)";break;case "float":case "number":case "numeric":v=String(v).replace(k,"");v=isNaN(Number(v))?"0":v;C="parseFloat("+C+")";v="parseFloat("+v+")";break;case "date":case "datetime":v=String(a.jgrid.parseDate(B.newfmt||"Y-m-d",v).getTime());C='jQuery.jgrid.parseDate("'+B.srcfmt+'",'+C+").getTime()";break;default:C=f._getStr(C);v=f._getStr('"'+
f._toStr(v)+'"')}f._append(C+" "+y+" "+v);f._setCommand(m,p);f._resetNegate();return f};this.equals=function(m,p,v){return f._compareValues(f.equals,m,p,"==",v)};this.greater=function(m,p,v){return f._compareValues(f.greater,m,p,">",v)};this.less=function(m,p,v){return f._compareValues(f.less,m,p,"<",v)};this.greaterOrEquals=function(m,p,v){return f._compareValues(f.greaterOrEquals,m,p,">=",v)};this.lessOrEquals=function(m,p,v){return f._compareValues(f.lessOrEquals,m,p,"<=",v)};this.startsWith=function(m,
p){var v=p===undefined||p===null?m:p;v=j?a.trim(v.toString()).length:v.toString().length;if(t)f._append(f._getStr("this."+m)+".substr(0,"+v+") == "+f._getStr('"'+f._toStr(p)+'"'));else{v=j?a.trim(p.toString()).length:p.toString().length;f._append(f._getStr("this")+".substr(0,"+v+") == "+f._getStr('"'+f._toStr(m)+'"'))}f._setCommand(f.startsWith,m);f._resetNegate();return f};this.endsWith=function(m,p){var v=p===undefined||p===null?m:p;v=j?a.trim(v.toString()).length:v.toString().length;t?f._append(f._getStr("this."+
m)+".substr("+f._getStr("this."+m)+".length-"+v+","+v+') == "'+f._toStr(p)+'"'):f._append(f._getStr("this")+".substr("+f._getStr("this")+'.length-"'+f._toStr(m)+'".length,"'+f._toStr(m)+'".length) == "'+f._toStr(m)+'"');f._setCommand(f.endsWith,m);f._resetNegate();return f};this.contains=function(m,p){t?f._append(f._getStr("this."+m)+'.indexOf("'+f._toStr(p)+'",0) > -1'):f._append(f._getStr("this")+'.indexOf("'+f._toStr(m)+'",0) > -1');f._setCommand(f.contains,m);f._resetNegate();return f};this.groupBy=
function(m,p,v,y){if(!f._hasData())return null;return f._getGroup(g,m,p,v,y)};this.orderBy=function(m,p,v,y){p=p===undefined||p===null?"a":a.trim(p.toString().toLowerCase());if(v===null||v===undefined)v="text";if(y===null||y===undefined)y="Y-m-d";if(p=="desc"||p=="descending")p="d";if(p=="asc"||p=="ascending")p="a";q.push({by:m,dir:p,type:v,datefmt:y});return f};return f})(e,null)},extend:function(e){a.extend(a.fn.jqGrid,e);this.no_legacy_api||a.fn.extend(e)}});a.fn.jqGrid=function(e){if(typeof e==
"string"){var d=a.jgrid.getAccessor(a.fn.jqGrid,e);if(!d)throw"jqGrid - No such method: "+e;var b=a.makeArray(arguments).slice(1);return d.apply(this,b)}return this.each(function(){if(!this.grid){var f=a.extend(true,{url:"",height:150,page:1,rowNum:20,rowTotal:null,records:0,pager:"",pgbuttons:true,pginput:true,colModel:[],rowList:[],colNames:[],sortorder:"asc",sortname:"",datatype:"xml",mtype:"GET",altRows:false,selarrrow:[],savedRow:[],shrinkToFit:true,xmlReader:{},jsonReader:{},subGrid:false,subGridModel:[],
...
else{this.p=f;var h,j,c;if(this.p.colNames.length===0)for(h=0;h<this.p.colModel.length;h++)this.p.colNames[h]=this.p.colModel[h].label||this.p.colModel[h].name;if(this.p.colNames.length!==this.p.colModel.length)alert(a.jgrid.errors.model);else{var k=a("<div class='ui-jqgrid-view'></div>"),l,o=a.browser.msie?true:false,n=a.browser.safari?true:false;c=this;c.p.direction=a.trim(c.p.direction.toLowerCase());if(a.inArray(c.p.direction,["ltr","rtl"])==-1)c.p.direction="ltr";j=c.p.direction;a(k).insertBefore(this);
...
this.id+"'>"+this.p.loadtext+"</div>").insertBefore(k);a(this).attr({cellSpacing:"0",cellPadding:"0",border:"0",role:"grid","aria-multiselectable":!!this.p.multiselect,"aria-labelledby":"gbox_"+this.id});var q=function(s,u){s=parseInt(s,10);return isNaN(s)?u?u:0:s},t=function(s,u,x){var E=c.p.colModel[s],D=E.align,U='style="',L=E.classes,P=E.name;if(D)U+="text-align:"+D+";";if(E.hidden===true)U+="display:none;";if(u===0)U+="width: "+g.headers[s].width+"px;";U+='"'+(L!==undefined?' class="'+L+'"':
"")+(E.title&&x?' title="'+a.jgrid.stripHtml(x)+'"':"");U+=' aria-describedby="'+c.p.id+"_"+P+'"';return U},m=function(s){return s===undefined||s===null||s===""?"&#160;":c.p.autoencode?a.jgrid.htmlEncode(s):s+""},p=function(s,u,x,E,D){x=c.p.colModel[x];if(typeof x.formatter!=="undefined"){s={rowId:s,colModel:x,gid:c.p.id};u=a.isFunction(x.formatter)?x.formatter.call(c,u,s,E,D):a.fmatter?a.fn.fmatter(x.formatter,u,s,E,D):m(u)}else u=m(u);return u},v=function(s,u,x,E,D){s=p(s,u,x,D,"add");return'<td role="gridcell" '+
...
var Ja={},La="";if(c.p.grouping&&c.p.groupingView.groupCollapse===true)La=' style="display:none;"';for(;wa<Ea;){ja=s[wa];ia=W(ja,Ka+wa);J=x===0?0:x+1;J=(J+wa)%2==1?Ha:"";sa.push("<tr"+La+' id="'+ia+'" role="row" class ="ui-widget-content jqgrow ui-row-'+c.p.direction+""+J+'">');if(c.p.rownumbers===true){sa.push(B(0,wa,c.p.page,c.p.rowNum));ha=1}if(c.p.multiselect===true){sa.push(y(ia,ha,wa));la=1}if(c.p.subGrid===true){sa.push(a(c).jqGrid("addSubGridCell",la+ha,wa+x));T=1}if(c.p.xmlReader.repeatitems){qa||
(qa=I(la+T+ha));var Ma=a(c.p.xmlReader.cell,ja);a.each(qa,function(Da){var va=Ma[this];if(!va)return false;K=va.textContent||va.text;pa[c.p.colModel[Da+la+T+ha].name]=K;sa.push(v(ia,K,Da+la+T+ha,wa+x,ja))})}else for(J=0;J<oa.length;J++){K=a(oa[J],ja).text();pa[c.p.colModel[J+la+T+ha].name]=K;sa.push(v(ia,K,J+la+T+ha,wa+x,ja))}sa.push("</tr>");if(c.p.grouping){J=c.p.groupingView.groupField.length;ca=[];for(z=0;z<J;z++)ca.push(pa[c.p.groupingView.groupField[z]]);Ja=a(c).jqGrid("groupingPrepare",sa,
ca,Ja,pa);sa=[]}if(L){pa[P]=ia;c.p.data.push(pa)}if(c.p.gridview===false){if(c.p.treeGrid===true){J=c.p.treeANode>=-1?c.p.treeANode:0;ca=a(sa.join(""))[0];a(c.rows[wa+J+x]).after(ca);try{a(c).jqGrid("setTreeNode",pa,ca)}catch(Na){}}else a("tbody:first",u).append(sa.join(""));if(c.p.subGrid===true)try{a(c).jqGrid("addSubGrid",c.rows[c.rows.length-1],la+ha)}catch(Oa){}D&&c.p.afterInsertRow.call(c,ia,pa,ja);sa=[]}pa={};S++;wa++;if(S==Fa)break}}if(c.p.gridview===true)if(c.p.grouping){a(c).jqGrid("groupingRender",
Ja,qa.length+la+T+ha);Ja=null}else a("tbody:first",u).append(sa.join(""));c.p.totaltime=new Date-U;if(S>0)if(c.p.records===0)c.p.records=Ea;sa=null;if(!c.p.treeGrid&&!c.p.scroll)c.grid.bDiv.scrollTop=0;c.p.reccount=S;c.p.treeANode=-1;c.p.userDataOnFooter&&a(c).jqGrid("footerData","set",c.p.userData,true);if(L){c.p.records=Ea;c.p.lastpage=Math.ceil(Ea/Fa)}E||O(false,true);if(L){for(;S<Ea;){ja=s[S];ia=W(ja,S);if(c.p.xmlReader.repeatitems){qa||(qa=I(la+T+ha));Ma=a(c.p.xmlReader.cell,ja);a.each(qa,function(Da){var va=
Ma[this];if(!va)return false;K=va.textContent||va.text;pa[c.p.colModel[Da+la+T+ha].name]=K})}else for(J=0;J<oa.length;J++){K=a(oa[J],ja).text();pa[c.p.colModel[J+la+T+ha].name]=K}pa[P]=ia;c.p.data.push(pa);pa={};S++}V()}}},G=function(s,u,x,E,D){var U=new Date;if(s){if(c.p.treeANode===-1&&!c.p.scroll){w(u,false);x=1}else x=x>1?x:1;var L,P,W=c.p.datatype!="local"&&c.p.loadonce||c.p.datatype=="jsonstring";if(W){c.p.data=[];c.p._index={};L=c.p.localReader.id="_id_"}c.p.reccount=0;if(c.p.datatype=="local"){u=
c.p.localReader;P="local"}else{u=c.p.jsonReader;P="json"}var J=0,S,K,ca,la=[],T,ha=0,ma=0,oa=0,qa,pa,ja={},ia;ca=[];var sa=c.p.altRows===true?" "+c.p.altclass:"";c.p.page=a.jgrid.getAccessor(s,u.page)||0;qa=a.jgrid.getAccessor(s,u.total);c.p.lastpage=qa===undefined?1:qa;c.p.records=a.jgrid.getAccessor(s,u.records)||0;c.p.userData=a.jgrid.getAccessor(s,u.userdata)||{};u.repeatitems||(T=la=C(P));P=c.p.keyIndex===false?u.id:c.p.keyIndex;if(la.length>0&&!isNaN(P)){if(c.p.remapColumns&&c.p.remapColumns.length)P=
a.inArray(P,c.p.remapColumns);P=la[P]}if(pa=a.jgrid.getAccessor(s,u.root)){qa=pa.length;s=0;var Ha=parseInt(c.p.rowNum,10),Ea=c.p.scroll?(parseInt(c.p.page,10)-1)*Ha+1:1;if(D)Ha*=D+1;var wa=a.isFunction(c.p.afterInsertRow),Fa={},Ka="";if(c.p.grouping&&c.p.groupingView.groupCollapse===true)Ka=' style="display:none;"';for(;s<qa;){D=pa[s];ia=a.jgrid.getAccessor(D,P);if(ia===undefined){ia=Ea+s;if(la.length===0)if(u.cell){S=D[u.cell];ia=S[P]||ia}}S=x===1?0:x;S=(S+s)%2==1?sa:"";ca.push("<tr"+Ka+' id="'+
ia+'" role="row" class= "ui-widget-content jqgrow ui-row-'+c.p.direction+""+S+'">');if(c.p.rownumbers===true){ca.push(B(0,s,c.p.page,c.p.rowNum));oa=1}if(c.p.multiselect){ca.push(y(ia,oa,s));ha=1}if(c.p.subGrid){ca.push(a(c).jqGrid("addSubGridCell",ha+oa,s+x));ma=1}if(u.repeatitems){if(u.cell)D=a.jgrid.getAccessor(D,u.cell);T||(T=I(ha+ma+oa))}for(K=0;K<T.length;K++){S=a.jgrid.getAccessor(D,T[K]);ca.push(v(ia,S,K+ha+ma+oa,s+x,D));ja[c.p.colModel[K+ha+ma+oa].name]=S}ca.push("</tr>");if(c.p.grouping){S=
...
c.p.afterInsertRow.call(c,ia,ja,D);ca=[]}ja={};J++;s++;if(J==Ha)break}if(c.p.gridview===true)c.p.grouping?a(c).jqGrid("groupingRender",Fa,T.length+ha+ma+oa):a("#"+c.p.id+" tbody:first").append(ca.join(""));c.p.totaltime=new Date-U;if(J>0)if(c.p.records===0)c.p.records=qa;if(!c.p.treeGrid&&!c.p.scroll)c.grid.bDiv.scrollTop=0;c.p.reccount=J;c.p.treeANode=-1;c.p.userDataOnFooter&&a(c).jqGrid("footerData","set",c.p.userData,true);if(W){c.p.records=qa;c.p.lastpage=Math.ceil(qa/Ha)}E||O(false,true);if(W){for(;J<
qa;){D=pa[J];ia=a.jgrid.getAccessor(D,P);if(ia===undefined){ia=Ea+J;if(la.length===0)if(u.cell){S=D[u.cell];ia=S[P]||ia}}if(D){if(u.repeatitems){if(u.cell)D=a.jgrid.getAccessor(D,u.cell);T||(T=I(ha+ma+oa))}for(K=0;K<T.length;K++){S=a.jgrid.getAccessor(D,T[K]);ja[c.p.colModel[K+ha+ma+oa].name]=S}ja[L]=ia;c.p.data.push(ja);ja={}}J++}V()}}}},M=function(){var s,u=false,x=grtypes=[],E,D;if(c.p.data.length){var U=c.p.grouping?c.p.groupingView:false;a.each(c.p.colModel,function(){sorttype=this.sorttype||
...
bw:function(T){return T.startsWith},bn:function(T){return T.not().startsWith},en:function(T){return T.not().endsWith},ew:function(T){return T.endsWith},ni:function(T){return T.not().equals},"in":function(T){return T.equals}},P=a.jgrid.from(c.p.data);if(c.p.search===true){var W=c.p.postData.filters;if(W){if(typeof W=="string")W=a.jgrid.parse(W);for(var J=0,S=W.rules.length,K;J<S;J++){K=W.rules[J];var ca=W.groupOp;if(L[K.op]&&K.field&&K.data&&ca)P=ca.toUpperCase()=="OR"?L[K.op](P)(K.field,K.data,x[K.field]).or():
L[K.op](P)(K.field,K.data,x[K.field])}}else try{P=L[c.p.postData.searchOper](P)(c.p.postData.searchField,c.p.postData.searchString,x[c.p.postData.searchField])}catch(la){}}if(c.p.grouping){P.orderBy(U.groupField[0],U.groupOrder[0],grtypes[0].stype,grtypes[0].srcfmt);U.groupDataSorted=true}if(s&&c.p.sortorder&&u)c.p.sortorder.toUpperCase()=="DESC"?P.orderBy(s,"d",x[s].stype,x[s].srcfmt):P.orderBy(s,"a",x[s].stype,x[s].srcfmt);L=P.select();P=parseInt(c.p.rowNum,10);W=L.length;J=parseInt(c.p.page,10);
S=Math.ceil(W/P);K={};L=L.slice((J-1)*P,J*P);x=P=null;K[c.p.localReader.total]=S;K[c.p.localReader.page]=J;K[c.p.localReader.records]=W;K[c.p.localReader.root]=L;L=null;return K}}},O=function(s,u){var x,E,D,U,L,P,W,J="";D=parseInt(c.p.page,10)-1;if(D<0)D=0;D*=parseInt(c.p.rowNum,10);L=D+c.p.reccount;if(c.p.scroll){x=a("tbody:first > tr:gt(0)",c.grid.bDiv);D=L-x.length;c.p.reccount=x.length;if(E=x.outerHeight()||c.grid.prevRowHeight){x=D*E;E=parseInt(c.p.records,10)*E;a(">div:first",c.grid.bDiv).css({height:E}).children("div:first").css({height:x,
display:x?"":"none"})}c.grid.bDiv.scrollLeft=c.grid.hDiv.scrollLeft}J=c.p.pager?c.p.pager:"";J+=c.p.toppager?J?","+c.p.toppager:c.p.toppager:"";if(J){W=a.jgrid.formatter.integer||{};x=q(c.p.page);E=q(c.p.lastpage);a(".selbox",J).attr("disabled",false);if(c.p.pginput===true){a(".ui-pg-input",J).val(c.p.page);a("#sp_1",J).html(a.fmatter?a.fmatter.util.NumberFormat(c.p.lastpage,W):c.p.lastpage)}if(c.p.viewrecords)if(c.p.reccount===0)a(".ui-paging-info",J).html(c.p.emptyrecords);else{U=D+1;P=c.p.records;
if(a.fmatter){U=a.fmatter.util.NumberFormat(U,W);L=a.fmatter.util.NumberFormat(L,W);P=a.fmatter.util.NumberFormat(P,W)}a(".ui-paging-info",J).html(a.jgrid.format(c.p.recordtext,U,L,P))}if(c.p.pgbuttons===true){if(x<=0)x=E=0;if(x==1||x===0){a("#first, #prev",c.p.pager).addClass("ui-state-disabled").removeClass("ui-state-hover");c.p.toppager&&a("#first_t, #prev_t",c.p.toppager).addClass("ui-state-disabled").removeClass("ui-state-hover")}else{a("#first, #prev",c.p.pager).removeClass("ui-state-disabled");
...
1+S)});u&&c.p.jqgdnd&&a(c).jqGrid("gridDnD","updateDnD");a.isFunction(c.p.gridComplete)&&c.p.gridComplete.call(c)},Q=function(s){if(!c.grid.hDiv.loading){var u=c.p.scroll&&s===false,x={},E,D=c.p.prmNames;if(c.p.page<=0)c.p.page=1;if(D.search!==null)x[D.search]=c.p.search;if(D.nd!==null)x[D.nd]=(new Date).getTime();if(D.rows!==null)x[D.rows]=c.p.rowNum;if(D.page!==null)x[D.page]=c.p.page;if(D.sort!==null)x[D.sort]=c.p.sortname;if(D.order!==null)x[D.order]=c.p.sortorder;if(c.p.rowTotal!==null&&D.totalrows!==
null)x[D.totalrows]=c.p.rowTotal;var U=c.p.loadComplete,L=a.isFunction(U);L||(U=null);var P=0;s=s||1;if(s>1)if(D.npage!==null){x[D.npage]=s;P=s-1;s=1}else U=function(J){c.p.page++;c.grid.hDiv.loading=false;L&&c.p.loadComplete.call(c,J);Q(s-1)};else D.npage!==null&&delete c.p.postData[D.npage];if(c.p.grouping){a(c).jqGrid("groupingSetup");if(c.p.groupingView.groupDataSorted==true)x[D.sort]=c.p.groupingView.groupField[0]+" "+c.p.groupingView.groupOrder[0]+", "+x[D.sort]}a.extend(c.p.postData,x);var W=
!c.p.scroll?1:c.rows.length-1;if(a.isFunction(c.p.datatype))c.p.datatype.call(c,c.p.postData,"load_"+c.p.id);else{a.isFunction(c.p.beforeRequest)&&c.p.beforeRequest.call(c);E=c.p.datatype.toLowerCase();switch(E){case "json":case "jsonp":case "xml":case "script":a.ajax(a.extend({url:c.p.url,type:c.p.mtype,dataType:E,data:a.isFunction(c.p.serializeGridData)?c.p.serializeGridData.call(c,c.p.postData):c.p.postData,success:function(J){E==="xml"?A(J,c.grid.bDiv,W,s>1,P):G(J,c.grid.bDiv,W,s>1,P);U&&U.call(c,
J);u&&c.grid.populateVisible();if(c.p.loadonce||c.p.treeGrid)c.p.datatype="local";ga()},error:function(J,S,K){a.isFunction(c.p.loadError)&&c.p.loadError.call(c,J,S,K);ga()},beforeSend:function(J){da();a.isFunction(c.p.loadBeforeSend)&&c.p.loadBeforeSend.call(c,J)}},a.jgrid.ajaxOptions,c.p.ajaxGridOptions));break;case "xmlstring":da();x=a.jgrid.stringToDoc(c.p.datastr);L&&c.p.loadComplete.call(c,x);A(x,c.grid.bDiv);c.p.datatype="local";c.p.datastr=null;ga();break;case "jsonstring":da();x=typeof c.p.datastr==
"string"?a.jgrid.parse(c.p.datastr):c.p.datastr;L&&c.p.loadComplete.call(c,x);G(x,c.grid.bDiv);c.p.datatype="local";c.p.datastr=null;ga();break;case "local":case "clientside":da();c.p.datatype="local";x=M();G(x,c.grid.bDiv,W,s>1,P);U&&U.call(c,x);u&&c.grid.populateVisible();ga();break}}}},V=function(){var s=c.p.data.length,u,x,E;u=c.p.rownumbers===true?1:0;x=c.p.multiselect===true?1:0;E=c.p.subGrid===true?1:0;u=c.p.keyIndex===false||c.p.loadonce===true?c.p.localReader.id:c.p.colModel[c.p.keyIndex+
...
u){var x="",E="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>",D="",U,L,P,W,J=function(S){var K;if(a.isFunction(c.p.onPaging))K=c.p.onPaging.call(c,S);c.p.selrow=null;if(c.p.multiselect){c.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.hDiv).attr("checked",false)}c.p.savedRow=[];if(K=="stop")return false;return true};s=s.substr(1);U="pg_"+s;L=s+"_left";P=s+"_center";W=s+"_right";a("#"+s).append("<div id='"+U+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;' role='row'><tbody><tr><td id='"+
L+"' align='left'></td><td id='"+P+"' align='center' style='white-space:pre;'></td><td id='"+W+"' align='right'></td></tr></tbody></table></div>").attr("dir","ltr");if(c.p.rowList.length>0){D="<td dir='"+j+"'>";D+="<select class='ui-pg-selbox' role='listbox'>";for(L=0;L<c.p.rowList.length;L++)D+="<option role='option' value='"+c.p.rowList[L]+"'"+(c.p.rowNum==c.p.rowList[L]?" selected":"")+">"+c.p.rowList[L]+"</option>";D+="</select></td>"}if(j=="rtl")E+=D;if(c.p.pginput===true)x="<td dir='"+j+"'>"+
...
"#"+U).bind("change",function(){c.p.page=Math.round(c.p.rowNum*(c.p.page-1)/this.value-0.5)+1;c.p.rowNum=this.value;if(u)a(".ui-pg-selbox",c.p.pager).val(this.value);else c.p.toppager&&a(".ui-pg-selbox",c.p.toppager).val(this.value);if(!J("records"))return false;Q();return false});if(c.p.pgbuttons===true){a(".ui-pg-button","#"+U).hover(function(){if(a(this).hasClass("ui-state-disabled"))this.style.cursor="default";else{a(this).addClass("ui-state-hover");this.style.cursor="pointer"}},function(){if(!a(this).hasClass("ui-state-disabled")){a(this).removeClass("ui-state-hover");
...
K;ca=true}if(ca){if(!J(this.id))return false;Q()}return false})}c.p.pginput===true&&a("input.ui-pg-input","#"+U).keypress(function(S){if((S.charCode?S.charCode:S.keyCode?S.keyCode:0)==13){c.p.page=a(this).val()>0?a(this).val():c.p.page;if(!J("user"))return false;Q();return false}return this})};var ua=function(s,u,x,E){if(c.p.colModel[u].sortable)if(!(c.p.savedRow.length>0)){if(!x){if(c.p.lastsort==u)if(c.p.sortorder=="asc")c.p.sortorder="desc";else{if(c.p.sortorder=="desc")c.p.sortorder="asc"}else c.p.sortorder=
...
true);c.grid.hDiv.scrollLeft=x}c.p.subGrid&&c.p.datatype=="local"&&a("td.sgexpanded","#"+c.p.id).each(function(){a(this).trigger("click")});Q();c.p.lastsort=u;if(c.p.sortname!=s&&u)c.p.lastsort=u}},za=function(s){var u=s,x;for(x=s+1;x<c.p.colModel.length;x++)if(c.p.colModel[x].hidden!==true){u=x;break}return u-s},Aa=function(s){var u,x={},E=n?0:c.p.cellLayout;for(u=x[0]=x[1]=x[2]=0;u<=s;u++)if(c.p.colModel[u].hidden===false)x[0]+=c.p.colModel[u].width+E;if(c.p.direction=="rtl")x[0]=c.p.width-x[0];
...
cell:"cell"}},c.p.jsonReader);c.p.localReader=a.extend(true,{root:"rows",page:"page",total:"total",records:"records",repeatitems:false,cell:"cell",id:"id",userdata:"userdata",subgrid:{root:"rows",repeatitems:true,cell:"cell"}},c.p.localReader);if(c.p.scroll){c.p.pgbuttons=false;c.p.pginput=false;c.p.rowList=[]}c.p.data.length&&V();var H="<thead><tr class='ui-jqgrid-labels' role='rowheader'>",Z,N,ea,X,Y,aa,R,ka;N=ka="";if(c.p.shrinkToFit===true&&c.p.forceFit===true)for(h=c.p.colModel.length-1;h>=0;h--)if(!c.p.colModel[h].hidden){c.p.colModel[h].resizable=
false;break}if(c.p.viewsortcols[1]=="horizontal"){ka=" ui-i-asc";N=" ui-i-desc"}Z=o?"class='ui-th-div-ie'":"";ka="<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+ka+" ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+j+"'></span>";ka+="<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+N+" ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+j+"'></span></span>";for(h=0;h<this.p.colNames.length;h++){N=c.p.headertitles?' title="'+a.jgrid.stripHtml(c.p.colNames[h])+
...
c.p.id+"_]").attr("checked",false);a(c.rows).each(function(s){if(!a(this).hasClass("subgrid")){a(this).removeClass("ui-state-highlight").attr("aria-selected","false");ba[s]=this.id}});c.p.selarrrow=[];c.p.selrow=null;ta=false}if(a.isFunction(c.p.onSelectAll))c.p.onSelectAll.call(c,ta?c.p.selarrrow:ba,ta)})}if(c.p.autowidth===true){H=a(r).innerWidth();c.p.width=H>0?H:"nw"}(function(){var s=0,u=c.p.cellLayout,x=0,E,D=c.p.scrollOffset,U,L=false,P,W=0,J=0,S=0,K;if(n)u=0;a.each(c.p.colModel,function(){if(typeof this.hidden===
"undefined")this.hidden=false;if(this.hidden===false){s+=q(this.width,0);if(this.fixed){W+=this.width;J+=this.width+u}else x++;S++}});if(isNaN(c.p.width))c.p.width=g.width=s;else g.width=c.p.width;c.p.tblwidth=s;if(c.p.shrinkToFit===false&&c.p.forceFit===true)c.p.forceFit=false;if(c.p.shrinkToFit===true&&x>0){P=g.width-u*x-J;if(!isNaN(c.p.height)){P-=D;L=true}s=0;a.each(c.p.colModel,function(ca){if(this.hidden===false&&!this.fixed){this.width=U=Math.round(P*this.width/(c.p.tblwidth-W));s+=U;E=ca}});
K=0;if(L){if(g.width-J-(s+u*x)!==D)K=g.width-J-(s+u*x)-D}else if(!L&&Math.abs(g.width-J-(s+u*x))!==1)K=g.width-J-(s+u*x);c.p.colModel[E].width+=K;c.p.tblwidth=s+K+W+S*u;if(c.p.tblwidth>c.p.width){c.p.colModel[E].width-=c.p.tblwidth-parseInt(c.p.width,10);c.p.tblwidth=c.p.width}}})();a(r).css("width",g.width+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+c.p.id+"'>&#160;</div>");a(k).css("width",g.width+"px");H=a("thead:first",c).get(0);var fa="";if(c.p.footerrow)fa+="<table role='grid' style='width:"+
...
a(this).dblclick(function(s){aa=s.target;R=a(aa,c.rows).closest("tr.jqgrow");if(a(R).length===0)return false;Ca=R[0].rowIndex;ra=a.jgrid.getCellIndex(aa);c.p.ondblClickRow.call(c,a(R).attr("id"),Ca,ra,s);return false});a.isFunction(this.p.onRightClickRow)&&a(this).bind("contextmenu",function(s){aa=s.target;R=a(aa,c.rows).closest("tr.jqgrow");if(a(R).length===0)return false;c.p.multiselect||a(c).jqGrid("setSelection",R[0].id,true);Ca=R[0].rowIndex;ra=a.jgrid.getCellIndex(aa);c.p.onRightClickRow.call(c,
a(R).attr("id"),Ca,ra,s);return false});g.bDiv=document.createElement("div");a(g.bDiv).append(a('<div style="position:relative;'+(o&&a.browser.version<8?"height:0.01%;":"")+'"></div>').append("<div></div>").append(this)).addClass("ui-jqgrid-bdiv").css({height:c.p.height+(isNaN(c.p.height)?"":"px"),width:g.width+"px"}).scroll(g.scrollGrid);a("table:first",g.bDiv).css({width:c.p.tblwidth+"px"});if(o){a("tbody",this).size()==2&&a("tbody:first",this).remove();c.p.multikey&&a(g.bDiv).bind("selectstart",
...
Ia;Q();na=false}c.p.gridstate="visible";a("#gbox_"+c.p.id).hasClass("ui-resizable")&&a(".ui-resizable-handle","#gbox_"+c.p.id).show();u&&c.p.onHeaderClick.call(c,c.p.gridstate,s)}return false});if(na){c.p.datatype="local";a(".ui-jqgrid-titlebar-close",g.cDiv).trigger("click")}}}else a(g.cDiv).hide();a(g.hDiv).after(g.bDiv).mousemove(function(s){if(g.resizing){g.dragMove(s);return false}});a(".ui-jqgrid-labels",g.hDiv).bind("selectstart",function(){return false});a(document).mouseup(function(){if(g.resizing){g.dragEnd();
return false}return true});c.formatCol=t;c.sortData=ua;c.updatepager=O;c.refreshIndex=V;c.formatter=function(s,u,x,E,D){return p(s,u,x,E,D)};a.extend(g,{populate:Q,emptyRows:w});this.grid=g;c.addXmlData=function(s){A(s,c.grid.bDiv)};c.addJSONData=function(s){G(s,c.grid.bDiv)};this.grid.cols=this.rows[0].cells;Q();c.p.hiddengrid=false;a(window).unload(function(){c=null})}}}})};a.jgrid.extend({getGridParam:function(e){var d=this[0];if(d&&d.grid)return e?typeof d.p[e]!="undefined"?d.p[e]:null:d.p},setGridParam:function(e){return this.each(function(){this.grid&&
typeof e==="object"&&a.extend(true,this.p,e)})},getDataIDs:function(){var e=[],d=0,b,f=0;this.each(function(){if((b=this.rows.length)&&b>0)for(;d<b;){if(a(this.rows[d]).hasClass("jqgrow")){e[f]=this.rows[d].id;f++}d++}});return e},setSelection:function(e,d){return this.each(function(){function b(j){var c=a(f.grid.bDiv)[0].clientHeight,k=a(f.grid.bDiv)[0].scrollTop,l=f.rows[j].offsetTop;j=f.rows[j].clientHeight;if(l+j>=c+k)a(f.grid.bDiv)[0].scrollTop=l-(c+k)+j+k;else if(l<c+k)if(l<k)a(f.grid.bDiv)[0].scrollTop=
l}var f=this,g,h;if(e!==undefined){d=d===false?false:true;if(g=f.rows.namedItem(e+"")){if(f.p.scrollrows===true){h=f.rows.namedItem(e).rowIndex;h>=0&&b(h)}if(f.p.multiselect){f.p.selrow=g.id;h=a.inArray(f.p.selrow,f.p.selarrrow);if(h===-1){g.className!=="ui-subgrid"&&a(g).addClass("ui-state-highlight").attr("aria-selected","true");g=true;a("#jqg_"+f.p.id+"_"+a.jgrid.jqID(f.p.selrow)).attr("checked",g);f.p.selarrrow.push(f.p.selrow);f.p.onSelectRow&&d&&f.p.onSelectRow.call(f,f.p.selrow,g)}else{g.className!==
"ui-subgrid"&&a(g).removeClass("ui-state-highlight").attr("aria-selected","false");g=false;a("#jqg_"+f.p.id+"_"+a.jgrid.jqID(f.p.selrow)).attr("checked",g);f.p.selarrrow.splice(h,1);f.p.onSelectRow&&d&&f.p.onSelectRow.call(f,f.p.selrow,g);g=f.p.selarrrow[0];f.p.selrow=g===undefined?null:g}}else if(g.className!=="ui-subgrid"){f.p.selrow&&a(f.rows.namedItem(f.p.selrow)).removeClass("ui-state-highlight").attr("aria-selected","false");f.p.selrow=g.id;a(g).addClass("ui-state-highlight").attr("aria-selected",
"true");f.p.onSelectRow&&d&&f.p.onSelectRow.call(f,f.p.selrow,true)}}}})},resetSelection:function(){return this.each(function(){var e=this,d;if(e.p.multiselect){a(e.p.selarrrow).each(function(b,f){d=e.rows.namedItem(f);a(d).removeClass("ui-state-highlight").attr("aria-selected","false");a("#jqg_"+e.p.id+"_"+a.jgrid.jqID(f)).attr("checked",false)});a("#cb_"+a.jgrid.jqID(e.p.id)).attr("checked",false);e.p.selarrrow=[]}else if(e.p.selrow){a("#"+e.p.id+" tbody:first tr#"+a.jgrid.jqID(e.p.selrow)).removeClass("ui-state-highlight").attr("aria-selected",
"false");e.p.selrow=null}e.p.savedRow=[]})},getRowData:function(e){var d={},b,f=false,g,h=0;this.each(function(){var j=this,c,k;if(typeof e=="undefined"){f=true;b=[];g=j.rows.length}else{k=j.rows.namedItem(e);if(!k)return d;g=2}for(;h<g;){if(f)k=j.rows[h];if(a(k).hasClass("jqgrow")){a("td",k).each(function(l){c=j.p.colModel[l].name;if(c!=="cb"&&c!=="subgrid"&&c!=="rn")if(j.p.treeGrid===true&&c==j.p.ExpandColumn)d[c]=a.jgrid.htmlDecode(a("span:first",this).html());else try{d[c]=a.unformat(this,{rowId:k.id,
colModel:j.p.colModel[l]},l)}catch(o){d[c]=a.jgrid.htmlDecode(a(this).html())}});if(f){b.push(d);d={}}}h++}});return b?b:d},delRowData:function(e){var d=false,b,f;this.each(function(){var g=this;if(b=g.rows.namedItem(e)){a(b).remove();g.p.records--;g.p.reccount--;g.updatepager(true,false);d=true;if(g.p.multiselect){f=a.inArray(e,g.p.selarrrow);f!=-1&&g.p.selarrrow.splice(f,1)}if(e==g.p.selrow)g.p.selrow=null}else return false;if(g.p.datatype=="local"){var h=g.p._index[e];if(typeof h!="undefined"){g.p.data.splice(h,
1);g.refreshIndex()}}if(g.p.altRows===true&&d){var j=g.p.altclass;a(g.rows).each(function(c){c%2==1?a(this).addClass(j):a(this).removeClass(j)})}});return d},setRowData:function(e,d,b){var f,g=true,h;this.each(function(){if(!this.grid)return false;var j=this,c,k,l=typeof b,o={};k=j.rows.namedItem(e);if(!k)return false;if(d)try{a(this.p.colModel).each(function(q){f=this.name;if(d[f]!==undefined){o[f]=this.formatter&&typeof this.formatter==="string"&&this.formatter=="date"?a.unformat.date(d[f],this):
d[f];c=j.formatter(e,d[f],q,d,"edit");h=this.title?{title:a.jgrid.stripHtml(c)}:{};j.p.treeGrid===true&&f==j.p.ExpandColumn?a("td:eq("+q+") > span:first",k).html(c).attr(h):a("td:eq("+q+")",k).html(c).attr(h)}});if(j.p.datatype=="local"){var n=j.p._index[e];if(typeof n!="undefined")j.p.data[n]=a.extend(true,j.p.data[n],o);o=null}}catch(r){g=false}if(g)if(l==="string")a(k).addClass(b);else l==="object"&&a(k).css(b)});return g},addRowData:function(e,d,b,f){b||(b="last");var g=false,h,j,c,k,l,o,n,r,
q="",t,m,p,v,y;if(d){if(a.isArray(d)){t=true;b="last";m=e}else{d=[d];t=false}this.each(function(){var B=this,C=d.length;l=B.p.rownumbers===true?1:0;c=B.p.multiselect===true?1:0;k=B.p.subGrid===true?1:0;if(!t)if(typeof e!="undefined")e+="";else{e=B.p.records+1+"";if(B.p.keyIndex!==false){m=B.p.colModel[B.p.keyIndex+c+k+l].name;if(typeof d[0][m]!="undefined")e=d[0][m]}}p=B.p.altclass;for(var I=0,w="",A={},G=a.isFunction(B.p.afterInsertRow)?true:false;I<C;){v=d[I];j="";if(t){try{e=v[m]}catch(M){e=B.p.records+
1+""}w=B.p.altRows===true?(B.rows.length-1)%2===0?p:"":""}if(l){q=B.formatCol(0,1,"");j+='<td role="gridcell" aria-describedby="'+B.p.id+'_rn" class="ui-state-default jqgrid-rownum" '+q+">0</td>"}if(c){r='<input role="checkbox" type="checkbox" id="jqg_'+B.p.id+"_"+e+'" class="cbox"/>';q=B.formatCol(l,1,"");j+='<td role="gridcell" aria-describedby="'+B.p.id+'_cb" '+q+">"+r+"</td>"}if(k)j+=a(B).jqGrid("addSubGridCell",c+l,1);for(n=c+k+l;n<B.p.colModel.length;n++){y=B.p.colModel[n];h=y.name;A[h]=y.formatter&&
typeof y.formatter==="string"&&y.formatter=="date"?a.unformat.date(v[h],y):v[h];r=B.formatter(e,v[h],n,v,"edit");q=B.formatCol(n,1,r);j+='<td role="gridcell" aria-describedby="'+B.p.id+"_"+h+'" '+q+">"+r+"</td>"}j='<tr id="'+e+'" role="row" class="ui-widget-content jqgrow ui-row-'+B.p.direction+" "+w+'">'+j+"</tr>";if(B.p.subGrid===true){j=a(j)[0];a(B).jqGrid("addSubGrid",j,c+l)}if(B.rows.length===0)a("table:first",B.grid.bDiv).append(j);else switch(b){case "last":a(B.rows[B.rows.length-1]).after(j);
break;case "first":a(B.rows[0]).after(j);break;case "after":if(o=B.rows.namedItem(f))a(B.rows[o.rowIndex+1]).hasClass("ui-subgrid")?a(B.rows[o.rowIndex+1]).after(j):a(o).after(j);break;case "before":if(o=B.rows.namedItem(f)){a(o).before(j);o=o.rowIndex}break}B.p.records++;B.p.reccount++;G&&B.p.afterInsertRow.call(B,e,v,v);I++;if(B.p.datatype=="local"){B.p._index[e]=B.p.data.length;B.p.data.push(A);A={}}}if(B.p.altRows===true&&!t)if(b=="last")(B.rows.length-1)%2==1&&a(B.rows[B.rows.length-1]).addClass(p);
else a(B.rows).each(function(O){O%2==1?a(this).addClass(p):a(this).removeClass(p)});B.updatepager(true,true);g=true})}return g},footerData:function(e,d,b){function f(k){for(var l in k)if(k.hasOwnProperty(l))return false;return true}var g,h=false,j={},c;if(typeof e=="undefined")e="get";if(typeof b!="boolean")b=true;e=e.toLowerCase();this.each(function(){var k=this,l;if(!k.grid||!k.p.footerrow)return false;if(e=="set")if(f(d))return false;h=true;a(this.p.colModel).each(function(o){g=this.name;if(e==
"set"){if(d[g]!==undefined){l=b?k.formatter("",d[g],o,d,"edit"):d[g];c=this.title?{title:a.jgrid.stripHtml(l)}:{};a("tr.footrow td:eq("+o+")",k.grid.sDiv).html(l).attr(c);h=true}}else if(e=="get")j[g]=a("tr.footrow td:eq("+o+")",k.grid.sDiv).html()})});return e=="get"?j:h},ShowHideCol:function(e,d){return this.each(function(){var b=this,f=false;if(b.grid){if(typeof e==="string")e=[e];d=d!="none"?"":"none";var g=d==""?true:false;a(this.p.colModel).each(function(h){if(a.inArray(this.name,e)!==-1&&this.hidden===
g){a("tr",b.grid.hDiv).each(function(){a("th:eq("+h+")",this).css("display",d)});a(b.rows).each(function(j){a("td:eq("+h+")",b.rows[j]).css("display",d)});b.p.footerrow&&a("td:eq("+h+")",b.grid.sDiv).css("display",d);if(d=="none")b.p.tblwidth-=this.width+b.p.cellLayout;else b.p.tblwidth+=this.width;this.hidden=!g;f=true}});if(f===true){a("table:first",b.grid.hDiv).width(b.p.tblwidth);a("table:first",b.grid.bDiv).width(b.p.tblwidth);b.grid.hDiv.scrollLeft=b.grid.bDiv.scrollLeft;if(b.p.footerrow){a("table:first",
b.grid.sDiv).width(b.p.tblwidth);b.grid.sDiv.scrollLeft=b.grid.bDiv.scrollLeft}b.p.shrinkToFit===true&&a(b).jqGrid("setGridWidth",b.grid.width-0.0010,true)}}})},hideCol:function(e){return this.each(function(){a(this).jqGrid("ShowHideCol",e,"none")})},showCol:function(e){return this.each(function(){a(this).jqGrid("ShowHideCol",e,"")})},remapColumns:function(e,d,b){function f(j){var c;c=j.length?a.makeArray(j):a.extend({},j);a.each(e,function(k){j[k]=c[this]})}function g(j,c){a(">tr"+(c||""),j).each(function(){var k=
this,l=a.makeArray(k.cells);a.each(e,function(){var o=l[this];o&&k.appendChild(o)})})}var h=this.get(0);f(h.p.colModel);f(h.p.colNames);f(h.grid.headers);g(a("thead:first",h.grid.hDiv),b&&":not(.ui-jqgrid-labels)");d&&g(a("#"+h.p.id+" tbody:first"),".jqgfirstrow, tr.jqgrow, tr.jqfoot");h.p.footerrow&&g(a("tbody:first",h.grid.sDiv));if(h.p.remapColumns)if(h.p.remapColumns.length)f(h.p.remapColumns);else h.p.remapColumns=a.makeArray(e);h.p.lastsort=a.inArray(h.p.lastsort,e);if(h.p.treeGrid)h.p.expColInd=
a.inArray(h.p.expColInd,e)},setGridWidth:function(e,d){return this.each(function(){if(this.grid){var b=this,f,g=0,h=b.p.cellLayout,j,c=0,k=false,l=b.p.scrollOffset,o,n=0,r=0,q=0,t;if(typeof d!="boolean")d=b.p.shrinkToFit;if(!isNaN(e)){e=parseInt(e,10);b.grid.width=b.p.width=e;a("#gbox_"+b.p.id).css("width",e+"px");a("#gview_"+b.p.id).css("width",e+"px");a(b.grid.bDiv).css("width",e+"px");a(b.grid.hDiv).css("width",e+"px");b.p.pager&&a(b.p.pager).css("width",e+"px");b.p.toppager&&a(b.p.toppager).css("width",
e+"px");if(b.p.toolbar[0]===true){a(b.grid.uDiv).css("width",e+"px");b.p.toolbar[1]=="both"&&a(b.grid.ubDiv).css("width",e+"px")}b.p.footerrow&&a(b.grid.sDiv).css("width",e+"px");if(d===false&&b.p.forceFit===true)b.p.forceFit=false;if(d===true){if(a.browser.safari)h=0;a.each(b.p.colModel,function(){if(this.hidden===false){g+=parseInt(this.width,10);if(this.fixed){r+=this.width;n+=this.width+h}else c++;q++}});if(c!==0){b.p.tblwidth=g;o=e-h*c-n;if(!isNaN(b.p.height))if(a(b.grid.bDiv)[0].clientHeight<
a(b.grid.bDiv)[0].scrollHeight||b.rows.length===1){k=true;o-=l}g=0;var m=b.grid.cols.length>0;a.each(b.p.colModel,function(p){if(this.hidden===false&&!this.fixed){f=Math.round(o*this.width/(b.p.tblwidth-r));if(!(f<0)){this.width=f;g+=f;b.grid.headers[p].width=f;b.grid.headers[p].el.style.width=f+"px";if(b.p.footerrow)b.grid.footers[p].style.width=f+"px";if(m)b.grid.cols[p].style.width=f+"px";j=p}}});t=0;if(k){if(e-n-(g+h*c)!==l)t=e-n-(g+h*c)-l}else if(Math.abs(e-n-(g+h*c))!==1)t=e-n-(g+h*c);b.p.colModel[j].width+=
t;b.p.tblwidth=g+t+r+h*q;if(b.p.tblwidth>e){k=b.p.tblwidth-parseInt(e,10);b.p.tblwidth=e;f=b.p.colModel[j].width-=k}else f=b.p.colModel[j].width;b.grid.headers[j].width=f;b.grid.headers[j].el.style.width=f+"px";if(m)b.grid.cols[j].style.width=f+"px";a("table:first",b.grid.bDiv).css("width",b.p.tblwidth+"px");a("table:first",b.grid.hDiv).css("width",b.p.tblwidth+"px");b.grid.hDiv.scrollLeft=b.grid.bDiv.scrollLeft;if(b.p.footerrow){b.grid.footers[j].style.width=f+"px";a("table:first",b.grid.sDiv).css("width",
b.p.tblwidth+"px")}}}}}})},setGridHeight:function(e){return this.each(function(){var d=this;if(d.grid){a(d.grid.bDiv).css({height:e+(isNaN(e)?"":"px")});d.p.height=e;d.p.scroll&&d.grid.populateVisible()}})},setCaption:function(e){return this.each(function(){this.p.caption=e;a("span.ui-jqgrid-title",this.grid.cDiv).html(e);a(this.grid.cDiv).show()})},setLabel:function(e,d,b,f){return this.each(function(){var g=this,h=-1;if(g.grid){if(isNaN(e))a(g.p.colModel).each(function(k){if(this.name==e){h=k;return false}});
else h=parseInt(e,10);if(h>=0){var j=a("tr.ui-jqgrid-labels th:eq("+h+")",g.grid.hDiv);if(d){var c=a(".s-ico",j);a("[id^=jqgh_]",j).empty().html(d).append(c);g.p.colNames[h]=d}if(b)typeof b==="string"?a(j).addClass(b):a(j).css(b);typeof f==="object"&&a(j).attr(f)}}})},setCell:function(e,d,b,f,g,h){return this.each(function(){var j=this,c=-1,k,l;if(j.grid){if(isNaN(d))a(j.p.colModel).each(function(n){if(this.name==d){c=n;return false}});else c=parseInt(d,10);if(c>=0)if(k=j.rows.namedItem(e)){var o=
a("td:eq("+c+")",k);if(b!==""||h===true){k=j.formatter(e,b,c,k,"edit");l=j.p.colModel[c].title?{title:a.jgrid.stripHtml(k)}:{};j.p.treeGrid&&a(".tree-wrap",a(o)).length>0?a("span",a(o)).html(k).attr(l):a(o).html(k).attr(l);if(j.p.datatype=="local"){k=j.p.colModel[c];b=k.formatter&&typeof k.formatter==="string"&&k.formatter=="date"?a.unformat.date(b,k):b;if(l=j.p._index[e])j.p.data[l][k.name]=b}}if(typeof f==="string")a(o).addClass(f);else f&&a(o).css(f);typeof g==="object"&&a(o).attr(g)}}})},getCell:function(e,
d){var b=false;this.each(function(){var f=this,g=-1;if(f.grid){if(isNaN(d))a(f.p.colModel).each(function(c){if(this.name===d){g=c;return false}});else g=parseInt(d,10);if(g>=0){var h=f.rows.namedItem(e);if(h)try{b=a.unformat(a("td:eq("+g+")",h),{rowId:h.id,colModel:f.p.colModel[g]},g)}catch(j){b=a.jgrid.htmlDecode(a("td:eq("+g+")",h).html())}}}});return b},getCol:function(e,d,b){var f=[],g,h=0;d=typeof d!="boolean"?false:d;if(typeof b=="undefined")b=false;this.each(function(){var j=this,c=-1;if(j.grid){if(isNaN(e))a(j.p.colModel).each(function(n){if(this.name===
e){c=n;return false}});else c=parseInt(e,10);if(c>=0){var k=j.rows.length,l=0;if(k&&k>0){for(;l<k;){if(a(j.rows[l]).hasClass("jqgrow")){try{g=a.unformat(a(j.rows[l].cells[c]),{rowId:j.rows[l].id,colModel:j.p.colModel[c]},c)}catch(o){g=a.jgrid.htmlDecode(j.rows[l].cells[c].innerHTML)}if(b)h+=parseFloat(g);else if(d)f.push({id:j.rows[l].id,value:g});else f[l]=g}l++}if(b)switch(b.toLowerCase()){case "sum":f=h;break;case "avg":f=h/k;break;case "count":f=k;break}}}}});return f},clearGridData:function(e){return this.each(function(){var d=
this;if(d.grid){if(typeof e!="boolean")e=false;if(d.p.deepempty)a("#"+d.p.id+" tbody:first tr:gt(0)").remove();else{var b=a("#"+d.p.id+" tbody:first tr:first")[0];a("#"+d.p.id+" tbody:first").empty().append(b)}d.p.footerrow&&e&&a(".ui-jqgrid-ftable td",d.grid.sDiv).html("&#160;");d.p.selrow=null;d.p.selarrrow=[];d.p.savedRow=[];d.p.records=0;d.p.page="0";d.p.lastpage="0";d.p.reccount=0;d.p.data=[];d.p_index={};d.updatepager(true,false)}})},getInd:function(e,d){var b=false,f;this.each(function(){if(f=
this.rows.namedItem(e))b=d===true?f:f.rowIndex});return b}})})(jQuery);
...
a.jgrid.htmlDecode(h);if(!f.p.colModel[d].edittype)f.p.colModel[d].edittype="text";f.p.savedRow.push({id:e,ic:d,name:g,v:h});if(a.isFunction(f.p.formatCell)){var k=f.p.formatCell(f.rows[e].id,g,h,e,d);if(k!==undefined)h=k}k=a.extend({},f.p.colModel[d].editoptions||{},{id:e+"_"+g,name:g});var l=createEl(f.p.colModel[d].edittype,k,h,true,a.extend({},a.jgrid.ajaxOptions,f.p.ajaxSelectOptions||{}));a.isFunction(f.p.beforeEditCell)&&f.p.beforeEditCell(f.rows[e].id,g,h,e,d);a(j).html("").append(l).attr("tabindex",
...
if(a.isFunction(b.p.beforeSubmitCell))(f=b.p.beforeSubmitCell(b.rows[e].id,k,h,e,d))||(f={});a("input.hasDatepicker",g).length>0&&a("input.hasDatepicker",g).datepicker("hide");if(b.p.cellsubmit=="remote")if(b.p.cellurl){var t={};if(b.p.autoencode)h=a.jgrid.htmlEncode(h);t[k]=h;n=b.p.prmNames;c=n.id;l=n.oper;t[c]=b.rows[e].id;t[l]=n.editoper;t=a.extend(f,t);a("#lui_"+b.p.id).show();b.grid.hDiv.loading=true;a.ajax(a.extend({url:b.p.cellurl,data:a.isFunction(b.p.serializeCellData)?b.p.serializeCellData(t):
t,type:"POST",complete:function(v,y){a("#lui_"+b.p.id).hide();b.grid.hDiv.loading=false;if(y=="success")if(a.isFunction(b.p.afterSubmitCell)){v=b.p.afterSubmitCell(v,t.id,k,h,e,d);if(v[0]===true){a(g).empty();a(b).jqGrid("setCell",b.rows[e].id,d,j,false,false,true);a(g).addClass("dirty-cell");a(b.rows[e]).addClass("edited");a.isFunction(b.p.afterSaveCell)&&b.p.afterSaveCell(b.rows[e].id,k,h,e,d);b.p.savedRow.splice(0,1)}else{info_dialog(a.jgrid.errors.errcap,v[1],a.jgrid.edit.bClose);a(b).jqGrid("restoreCell",
e,d)}}else{a(g).empty();a(b).jqGrid("setCell",b.rows[e].id,d,j,false,false,true);a(g).addClass("dirty-cell");a(b.rows[e]).addClass("edited");a.isFunction(b.p.afterSaveCell)&&b.p.afterSaveCell(b.rows[e].id,k,h,e,d);b.p.savedRow.splice(0,1)}},error:function(v,y){a("#lui_"+b.p.id).hide();b.grid.hDiv.loading=false;a.isFunction(b.p.errorCell)?b.p.errorCell(v,y):info_dialog(a.jgrid.errors.errcap,v.status+" : "+v.statusText+"<br/>"+y,a.jgrid.edit.bClose);a(b).jqGrid("restoreCell",e,d)}},a.jgrid.ajaxOptions,
b.p.ajaxCellOptions||{}))}else try{info_dialog(a.jgrid.errors.errcap,a.jgrid.errors.nourl,a.jgrid.edit.bClose);a(b).jqGrid("restoreCell",e,d)}catch(m){}if(b.p.cellsubmit=="clientArray"){a(g).empty();a(b).jqGrid("setCell",b.rows[e].id,d,j,false,false,true);a(g).addClass("dirty-cell");a(b.rows[e]).addClass("edited");a.isFunction(b.p.afterSaveCell)&&b.p.afterSaveCell(b.rows[e].id,k,h,e,d);b.p.savedRow.splice(0,1)}}else try{window.setTimeout(function(){info_dialog(a.jgrid.errors.errcap,h+" "+q[1],a.jgrid.edit.bClose)},
100);a(b).jqGrid("restoreCell",e,d)}catch(p){}}else a(b).jqGrid("restoreCell",e,d)}a.browser.opera?a("#"+b.p.knv).attr("tabindex","-1").focus():window.setTimeout(function(){a("#"+b.p.knv).attr("tabindex","-1").focus()},0)}})},restoreCell:function(e,d){return this.each(function(){var b=this,f;if(!(!b.grid||b.p.cellEdit!==true)){f=b.p.savedRow.length>=1?0:null;if(f!==null){var g=a("td:eq("+d+")",b.rows[e]);if(a.isFunction(a.fn.datepicker))try{a("input.hasDatepicker",g).datepicker("hide")}catch(h){}a(g).empty().attr("tabindex",
"-1");a(b).jqGrid("setCell",b.rows[e].id,d,b.p.savedRow[f].v,false,false,true);a.isFunction(b.p.afterRestoreCell)&&b.p.afterRestoreCell(b.rows[e].id,b.p.savedRow[f].v,e,d);b.p.savedRow.splice(0,1)}window.setTimeout(function(){a("#"+b.p.knv).attr("tabindex","-1").focus()},0)}})},nextCell:function(e,d){return this.each(function(){var b=this,f=false;if(!(!b.grid||b.p.cellEdit!==true)){for(var g=d+1;g<b.p.colModel.length;g++)if(b.p.colModel[g].editable===true){f=g;break}if(f!==false)a(b).jqGrid("editCell",
e,f,true);else b.p.savedRow.length>0&&a(b).jqGrid("saveCell",e,d)}})},prevCell:function(e,d){return this.each(function(){var b=this,f=false;if(!(!b.grid||b.p.cellEdit!==true)){for(var g=d-1;g>=0;g--)if(b.p.colModel[g].editable===true){f=g;break}if(f!==false)a(b).jqGrid("editCell",e,f,true);else b.p.savedRow.length>0&&a(b).jqGrid("saveCell",e,d)}})},GridNav:function(){return this.each(function(){function e(j,c,k){if(k.substr(0,1)=="v"){var l=a(b.grid.bDiv)[0].clientHeight,o=a(b.grid.bDiv)[0].scrollTop,
n=b.rows[j].offsetTop+b.rows[j].clientHeight,r=b.rows[j].offsetTop;if(k=="vd")if(n>=l)a(b.grid.bDiv)[0].scrollTop=a(b.grid.bDiv)[0].scrollTop+b.rows[j].clientHeight;if(k=="vu")if(r<o)a(b.grid.bDiv)[0].scrollTop=a(b.grid.bDiv)[0].scrollTop-b.rows[j].clientHeight}if(k=="h"){k=a(b.grid.bDiv)[0].clientWidth;l=a(b.grid.bDiv)[0].scrollLeft;o=b.rows[j].cells[c].offsetLeft;if(b.rows[j].cells[c].offsetLeft+b.rows[j].cells[c].clientWidth>=k+parseInt(l,10))a(b.grid.bDiv)[0].scrollLeft=a(b.grid.bDiv)[0].scrollLeft+
b.rows[j].cells[c].clientWidth;else if(o<l)a(b.grid.bDiv)[0].scrollLeft=a(b.grid.bDiv)[0].scrollLeft-b.rows[j].cells[c].clientWidth}}function d(j,c){var k,l;if(c=="lft"){k=j+1;for(l=j;l>=0;l--)if(b.p.colModel[l].hidden!==true){k=l;break}}if(c=="rgt"){k=j-1;for(l=j;l<b.p.colModel.length;l++)if(b.p.colModel[l].hidden!==true){k=l;break}}return k}var b=this;if(!(!b.grid||b.p.cellEdit!==true)){b.p.knv=b.p.id+"_kn";var f=a("<span style='width:0px;height:0px;background-color:black;' tabindex='0'><span tabindex='-1' style='width:0px;height:0px;background-color:grey' id='"+
...
this.triggerSearch=j;this.clearSearch=c}else alert("Could not get grid colModel")}else alert("No target grid is set!")})},filterToolbar:function(e){e=a.extend({autosearch:true,searchOnEnter:true,beforeSearch:null,afterSearch:null,beforeClear:null,afterClear:null,searchurl:"",stringResult:false,groupOp:"AND"},e||{});return this.each(function(){function d(j,c){var k=a(j);k[0]&&jQuery.each(c,function(){this.data!==undefined?k.bind(this.type,this.data,this.fn):k.bind(this.type,this.fn)})}var b=this,f=
function(){var j={},c=0,k,l,o={};a.each(b.p.colModel,function(){l=this.index||this.name;var p=this.searchoptions&&this.searchoptions.sopt?this.searchoptions.sopt[0]:"bw";switch(this.stype){case "select":if(k=a("select[name="+l+"]",b.grid.hDiv).val()){j[l]=k;o[l]=p;c++}else try{delete b.p.postData[l]}catch(v){}break;case "text":if(k=a("input[name="+l+"]",b.grid.hDiv).val()){j[l]=k;o[l]=p;c++}else try{delete b.p.postData[l]}catch(y){}break}});var n=c>0?true:false;if(e.stringResult===true||b.p.datatype==
"local"){var r='{"groupOp":"'+e.groupOp+'","rules":[',q=0;a.each(j,function(p,v){if(q>0)r+=",";r+='{"field":"'+p+'",';r+='"op":"'+o[p]+'",';r+='"data":"'+v+'"}';q++});r+="]}";a.extend(b.p.postData,{filters:r})}else a.extend(b.p.postData,j);var t;if(b.p.searchurl){t=b.p.url;a(b).jqGrid("setGridParam",{url:b.p.searchurl})}var m=false;if(a.isFunction(e.beforeSearch))m=e.beforeSearch.call(b);m||a(b).jqGrid("setGridParam",{search:n}).trigger("reloadGrid",[{page:1}]);t&&a(b).jqGrid("setGridParam",{url:t});
a.isFunction(e.afterSearch)&&e.afterSearch()},g=a("<tr class='ui-search-toolbar' role='rowheader'></tr>"),h;a.each(b.p.colModel,function(){var j=this,c,k,l,o;k=a("<th role='columnheader' class='ui-state-default ui-th-column ui-th-"+b.p.direction+"'></th>");c=a("<div style='width:100%;position:relative;height:100%;padding-right:0.3em;'></div>");this.hidden===true&&a(k).css("display","none");this.search=this.search===false?false:true;if(typeof this.stype=="undefined")this.stype="text";l=a.extend({},
...
m.themodal,{gbox:"#gbox_"+r,jqm:b.jqModal,jqM:false,closeoverlay:O,modal:b.modal});O||a(".jqmOverlay").click(function(){if(!h())return false;hideModal("#"+m.themodal,{gb:"#gbox_"+r,jqm:b.jqModal,onClose:e.onClose});return false});v&&v(a("#"+q))}else{a(n.p.colModel).each(function(){var F=this.formoptions;C=Math.max(C,F?F.colpos||0:0);I=Math.max(I,F?F.rowpos||0:0)});var Q=isNaN(b.dataheight)?b.dataheight:b.dataheight+"px",V;Q=a("<form name='FormPost' id='"+q+"' class='FormGrid' onSubmit='return false;' style='width:100%;overflow:auto;position:relative;height:"+
Q+";'></form>").data("disabled",false);var da=a("<table id='"+t+"' class='EditTable' cellspacing='0' cellpading='0' border='0'><tbody></tbody></table>");a(Q).append(da);V=a("<tr id='FormError' style='display:none'><td class='ui-state-error' colspan='"+C*2+"'></td></tr>");V[0].rp=0;a(da).append(V);V=a("<tr style='display:none' class='tinfo'><td class='topinfo' colspan='"+C*2+"'>"+e.topinfo+"</td></tr>");V[0].rp=0;a(da).append(V);y&&y(a("#"+q));V=(y=n.p.direction=="rtl"?true:false)?"nData":"pData";
...
H){a("tbody",da).append(H)})}b.gbox="#gbox_"+r;var Ga=false;if(b.closeOnEscape===true){b.closeOnEscape=false;Ga=true}Q=a("<span></span>").append(Q).append(V);createModal(m,Q,b,"#gview_"+n.p.id,a("#gbox_"+n.p.id)[0]);if(y){a("#pData, #nData","#"+t+"_2").css("float","right");a(".EditButton","#"+t+"_2").css("text-align","left")}e.topinfo&&a(".tinfo","#"+t+"_2").show();e.bottominfo&&a(".binfo","#"+t+"_2").show();V=Q=null;a("#"+m.themodal).keydown(function(F){var H=F.target;if(a("#"+q).data("disabled")===
...
a("#nData","#"+n+"_2").removeClass("ui-state-disabled")}function h(){var A=a(k).jqGrid("getDataIDs"),G=a("#id_g","#"+n).val();return[a.inArray(G,A),A]}function j(A,G,M,O){for(var Q,V,da,ga=0,ua,za,Aa=[],Ga=false,F="<td class='CaptionTD form-view-label ui-widget-content' width='"+b.labelswidth+"'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>",H="",Z=["integer","number","currency"],N=0,ea=0,X,Y,aa,R=1;R<=O;R++)H+=R==1?F:"<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>";
a(G.p.colModel).each(function(){V=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false;if(!V&&this.align==="right")if(this.formatter&&a.inArray(this.formatter,Z)!==-1)N=Math.max(N,parseInt(this.width,10));else ea=Math.max(ea,parseInt(this.width,10))});X=N!==0?N:ea!==0?ea:0;Ga=a(G).jqGrid("getInd",A);a(G.p.colModel).each(function(ka){Q=this.name;Y=false;za=(V=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false)?"style='display:none'":
"";aa=typeof this.viewable!="boolean"?true:this.viewable;if(Q!=="cb"&&Q!=="subgrid"&&Q!=="rn"&&aa){ua=Ga===false?"":Q==G.p.ExpandColumn&&G.p.treeGrid===true?a("td:eq("+ka+")",G.rows[Ga]).text():a("td:eq("+ka+")",G.rows[Ga]).html();Y=this.align==="right"&&X!==0?true:false;a.extend({},this.editoptions||{},{id:Q,name:Q});var ba=a.extend({},{rowabove:false,rowcontent:""},this.formoptions||{}),ta=parseInt(ba.rowpos,10)||ga+1,fa=parseInt((parseInt(ba.colpos,10)||1)*2,10);if(ba.rowabove){var xa=a("<tr><td class='contentinfo' colspan='"+
O*2+"'>"+ba.rowcontent+"</td></tr>");a(M).append(xa);xa[0].rp=ta}da=a(M).find("tr[rowpos="+ta+"]");if(da.length===0){da=a("<tr "+za+" rowpos='"+ta+"'></tr>").addClass("FormData").attr("id","trv_"+Q);a(da).append(H);a(M).append(da);da[0].rp=ta}a("td:eq("+(fa-2)+")",da[0]).html("<b>"+(typeof ba.label==="undefined"?G.p.colNames[ka]:ba.label)+"</b>");a("td:eq("+(fa-1)+")",da[0]).append("<span>"+ua+"</span>").attr("id","v_"+Q);Y&&a("td:eq("+(fa-1)+") span",da[0]).css({"text-align":"right",width:X+"px"});
Aa[ga]=ka;ga++}});if(ga>0){A=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(O*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='"+A+"'/></td></tr>");A[0].rp=ga+99;a(M).append(A)}return Aa}function c(A,G){var M,O,Q=0,V,da;if(da=a(G).jqGrid("getInd",A,true)){a("td",da).each(function(ga){M=G.p.colModel[ga].name;O=G.p.colModel[ga].editrules&&G.p.colModel[ga].editrules.edithidden===true?false:G.p.colModel[ga].hidden===true?
true:false;if(M!=="cb"&&M!=="subgrid"&&M!=="rn"){V=M==G.p.ExpandColumn&&G.p.treeGrid===true?a(this).text():a(this).html();a.extend({},G.p.colModel[ga].editoptions||{});M=a.jgrid.jqID("v_"+M);a("#"+M+" span","#"+n).html(V);O&&a("#"+M,"#"+n).parents("tr:first").hide();Q++}});Q>0&&a("#id_g","#"+n).val(A)}}var k=this;if(k.grid&&d){if(!b.imgpath)b.imgpath=k.p.imgpath;var l=k.p.id,o="ViewGrid_"+l,n="ViewTbl_"+l,r={themodal:"viewmod"+l,modalhead:"viewhd"+l,modalcontent:"viewcnt"+l,scrollelm:o},q=1,t=0;if(a("#"+
...
a("#"+b.p.id+" tbody:first").append(g);g=null;f.sortitems[0]=[];f.sortnames[0]=[];f.summaryval[0]=[]})},groupingGroupBy:function(e){return this.each(function(){var d=this;if(typeof e=="string")e=[e];var b=d.p.groupingView;d.p.grouping=true;for(var f=0;f<b.groupField.length;f++)b.groupColumnShow[f]||a(d).jqGrid("showCol",b.groupField[f]);b.groupField=e;a(d).trigger("reloadGrid")})},groupingRemove:function(e){return this.each(function(){var d=this;if(typeof e=="undefined")e=true;d.p.grouping=false;
e===true?a("tr.jqgroup, tr.jqfoot","#"+d.p.id+" tbody:first").remove():a(d).trigger("reloadGrid")})},groupingCalculations:{sum:function(e,d,b){return parseFloat(e||0)+parseFloat(b[d]||0)},min:function(e,d,b){if(e==="")return parseFloat(b[d]||0);return Math.min(parseFloat(e),parseFloat(b[d]||0))},max:function(e,d,b){if(e==="")return parseFloat(b[d]||0);return Math.max(parseFloat(e),parseFloat(b[d]||0))},count:function(e,d,b){if(e==="")e=0;return b.hasOwnProperty(d)?e+1:0},avg:function(e,d,b){return parseFloat(e||
0)+parseFloat(b[d]||0)}}})})(jQuery);
...
r++}}});if(r>0){t.id=e;l.p.savedRow.push(t);a(m).attr("editable","1");a("td:eq("+q+") input",m).focus();d===true&&a(m).bind("keydown",function(v){v.keyCode===27&&a(l).jqGrid("restoreRow",e,k);if(v.keyCode===13){if(v.target.tagName=="TEXTAREA")return true;a(l).jqGrid("saveRow",e,f,g,h,j,c,k);return false}v.stopPropagation()});a.isFunction(b)&&b(e)}}}})},saveRow:function(e,d,b,f,g,h,j){return this.each(function(){var c=this,k,l={},o={},n,r,q,t;if(c.grid){t=a(c).jqGrid("getInd",e,true);if(t!==false){n=
a(t).attr("editable");b=b?b:c.p.editurl;if(n==="1"){var m;a("td",t).each(function(B){m=c.p.colModel[B];k=m.name;if(k!="cb"&&k!="subgrid"&&m.editable===true&&k!="rn"){switch(m.edittype){case "checkbox":var C=["Yes","No"];if(m.editoptions)C=m.editoptions.value.split(":");l[k]=a("input",this).attr("checked")?C[0]:C[1];break;case "text":case "password":case "textarea":case "button":l[k]=a("input, textarea",this).val();break;case "select":if(m.editoptions.multiple){C=a("select",this);var I=[];l[k]=a(C).val();
l[k]=l[k]?l[k].join(","):"";a("select > option:selected",this).each(function(A,G){I[A]=a(G).text()});o[k]=I.join(",")}else{l[k]=a("select>option:selected",this).val();o[k]=a("select>option:selected",this).text()}if(m.formatter&&m.formatter=="select")o={};break;case "custom":try{if(m.editoptions&&a.isFunction(m.editoptions.custom_value)){l[k]=m.editoptions.custom_value(a(".customelement",this),"get");if(l[k]===undefined)throw"e2";}else throw"e1";}catch(w){w=="e1"&&info_dialog(jQuery.jgrid.errors.errcap,
...
c.p.savedRow.splice(r,1);a.isFunction(g)&&g(e,n)}else{a("#lui_"+c.p.id).show();a.ajax(a.extend({url:b,data:a.isFunction(c.p.serializeRowData)?c.p.serializeRowData(l):l,type:"POST",complete:function(B,C){a("#lui_"+c.p.id).hide();if(C==="success")if((a.isFunction(d)?d(B):true)===true){c.p.autoencode&&a.each(l,function(I,w){l[I]=a.jgrid.htmlDecode(w)});l=a.extend({},l,o);a(c).jqGrid("setRowData",e,l);a(t).attr("editable","0");for(C=0;C<c.p.savedRow.length;C++)if(c.p.savedRow[C].id==e){r=C;break}r>=0&&
c.p.savedRow.splice(r,1);a.isFunction(g)&&g(e,B)}else a(c).jqGrid("restoreRow",e,j)},error:function(B,C){a("#lui_"+c.p.id).hide();a.isFunction(h)?h(e,B,C):alert("Error Row: "+e+" Result: "+B.status+":"+B.statusText+" Status: "+C);a(c).jqGrid("restoreRow",e,j)}},a.jgrid.ajaxOptions,c.p.ajaxRowOptions||{}))}a(t).unbind("keydown")}}}}})},restoreRow:function(e,d){return this.each(function(){var b=this,f,g,h={};if(b.grid){g=a(b).jqGrid("getInd",e,true);if(g!==false){for(var j=0;j<b.p.savedRow.length;j++)if(b.p.savedRow[j].id==
e){f=j;break}if(f>=0){if(a.isFunction(a.fn.datepicker))try{a("input.hasDatepicker","#"+a.jgrid.jqID(g.id)).datepicker("hide")}catch(c){}a.each(b.p.colModel,function(){if(this.editable===true&&this.name in b.p.savedRow[f])h[this.name]=b.p.savedRow[f][this.name]});a(b).jqGrid("setRowData",e,h);a(g).attr("editable","0").unbind("keydown");b.p.savedRow.splice(f,1)}a.isFunction(d)&&d(e)}}})}})})(jQuery);
...
G+'","rules":[':{groupOp:G,rules:[]};w.find(".sf").each(function(M){var O=jQuery(this).find("select[name='field'] :selected").val(),Q=jQuery(this).find("select[name='op'] :selected").val(),V=jQuery(this).find("input.vdata,select.vdata :selected").val();V+="";V=V.replace(/\\/g,"\\\\").replace(/\"/g,'\\"');if(n.stringResult){if(M>0)A+=",";A+='{"field":"'+O+'",';A+='"op":"'+Q+'",';A+='"data":"'+V+'"}'}else A.rules.push({field:O,op:Q,data:V})});if(n.stringResult)A+="]}";n.onSearch(A);return false});b.find(".ui-reset").click(function(w,
...
var G=[],M,O,Q,V,da={};selDOMobj=A.find("select[name='field']")[0];M=0;for(Q=selDOMobj.options.length;M<Q;M++){da[selDOMobj.options[M].value]={index:M,ops:{}};G.push(selDOMobj.options[M].value)}M=0;for(Q=G.length;M<Q;M++){if(selDOMobj=A.find(".ops > select[class='field"+M+"']")[0]){O=0;for(V=selDOMobj.options.length;O<V;O++)da[G[M]].ops[selDOMobj.options[O].value]=O}if(selDOMobj=A.find(".data > select[class='field"+M+"']")[0]){da[G[M]].data={};O=0;for(V=selDOMobj.options.length;O<V;O++)da[G[M]].data[selDOMobj.options[O].value]=
O}}var ga,ua,za,Aa;G=w.field;if(da[G])ga=da[G].index;if(ga!=null){ua=da[G].ops[w.op];if(ua===undefined){M=0;for(Q=g.operators.length;M<Q;M++)if(g.operators[M].op==w.op){ua=M;break}}za=w.data;Aa=da[G].data==null?-1:da[G].data[za]}if(ga!=null&&ua!=null&&Aa!=null){A.find("select[name='field']")[0].selectedIndex=ga;A.find("select[name='field']").change();A.find("select[name='op']")[0].selectedIndex=ua;A.find("input.vdata").val(za);if(A=A.find("select.vdata")[0])A.selectedIndex=Aa;return true}else return false}}}
